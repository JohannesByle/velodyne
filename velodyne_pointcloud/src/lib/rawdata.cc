...
void RawData::setupAzimuthCache()
{
  ...
  std::tuple<float, float, float> RawData::unpackOffline(const velodyne_msgs::VelodynePacket &pkt, const ros::Time &scan_start_time) {
        using velodyne_pointcloud::LaserCorrection;
        ROS_DEBUG_STREAM("Received packet, time: " << pkt.stamp);
        ...
        for (int i = 0; i < BLOCKS_PER_PACKET; i++) {
            ...
            for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
                ...
                if ((block.rotation >= config_.min_angle
                    ...
                    return std::tuple<float, float, float>{x_coord, y_coord, z_coord};
                }
            }
        }
    }
